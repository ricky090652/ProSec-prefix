...
**目前進度**：**全量評測完成，論文安全性復現成功** —— LoRA all-linear 47.94 → **33.70**（論文 50.57 → 33.47，ON 只差 0.22 pt）· Pareto 前緣仍全是 LoRA
**上次更新**：2026-09-07（693 題 × 10 × 5 語言全量評測）
...
>
> **P-sven 結果**（2026-09-05，prefix nvt=8 + `--prefix_dropout 0.1`，其餘與 P-match 全同）：
>
> ### utility 隨 prefix 長度單調且陡峭
>
> | nvt | 參數 | `chosen early`（每 token） | 保留/token | margin late | acc late | **HumanEval Δ** | 退步/進步 |
> |---|---|---|---|---|---|---|---|
> | **8** | 1.57M | **−0.4505（98.0%）** | **96.8%** | 0.407 | 0.710 | **−4.27** | 16 / 9 |
> | 16 | 3.15M | −0.7700（96.7%） | 94.6% | 0.822 | 0.753 | −10.98 | 27 / 9 |
> | 64 | 12.6M | −4.4414（82.3%） | 87.9% | 1.323 | 0.740 | −32.93 | 59 / 5 |
>
> **砍一半長度買回 6.71 pt 的 utility。** 初始擾動、最終保留率、utility 三條線
> 全部單調且方向一致 —— **prefix 的 utility 代價幾乎全部來自那筆與長度成正比的初始擾動**，
> 這正是先前從 `chosen early` 推出的機制，現在有了結果面的佐證。
>
> ### 這修正了先前的結論
>
> 「prefix 傷 utility 傷得比 LoRA 重 2.6 倍」是在 **nvt=16** 上得到的，而 nvt=16
> 已經超出 SVEN 對 3.8B 模型的建議尺度（約 9~10）。在 SVEN 尺度下差距縮小很多：
>
> | | 參數 | utility Δ |
> |---|---|---|
> | LoRA qkv_proj | 3.1M | **+1.22** |
> | **prefix nvt=8** | **1.57M** | **−4.27** |
> | LoRA all-linear | 12.6M | −4.27 |
> | prefix nvt=16 | 3.15M | −10.98 |
>
> prefix nvt=8 用**一半的參數**達到與 LoRA all-linear 相同的 −4.27。
> 仍輸給 lora_qkv 的 +1.22（差 5.5 pt），但不再是 12.2 pt 的懸殊差距。
>
> ### 訓練動態：梯度更吵但可恢復
>
> `grad_norm > 500` 出現 **20/160**（nvt=16 是 5/160），最大 92,320（step 270）。
> 但每次尖峰後 chosen 都回升，最深只到 −1.999，最終穩在 −0.7312，
> 而且 step 555 之後沒有尖峰。`max_grad_norm 0.3` 夾住了，訓練可用。
> 頻率變 4 倍最可能來自 dropout 的隨機性（每步丟掉 10% 的 prefix 元素）。
>
> ### ⚠️ 未歸因的混淆
>
> nvt=16 **沒有** dropout、nvt=8 **有**，所以 −10.98 → −4.27 的改善
> **無法區分是「長度減半」還是「加了 dropout」**（也可能兩者都有貢獻）。
> 梯度尖峰頻率變 4 倍同樣分不出來源。
>
> - [x] **P-sven** prefix nvt=8 + dropout 0.1 → utility **−4.27**
> - [ ] **P-sven-sec** nvt=8 的安全性（與 lora_qkv 一起，這是最後兩個缺口）
> - [ ] **P-sven-attr** nvt=8 **無** dropout（2.5 小時）——歸因用，現在值得跑了
>
> **P-sven-sec 結果**（2026-09-06，100 題×5、c/cpp、`--max_new_tokens 2048`）：
>
> | | 參數 | 安全 Δ ↓ | utility Δ ↑ | 安全 pt/M參數 | 安全用的上限 | 退化守門 |
> |---|---|---|---|---|---|---|
> | LoRA all-linear | 12.6M | −17.80 | −4.27 | 1.41 | 1024* | ✅ |
> | **LoRA qkv_proj** | 3.15M | **−2.80** | **+1.22** | 0.89 | 2048 | ✅ |
> | **prefix nvt=8** | **1.57M** | **−5.20** | −4.27 | **3.31** | 2048 | ✅ |
> | prefix nvt=16 | 3.15M | −5.82 | −10.98 | 1.85 | 1024* | ❌ 語法 −4.74 |
> | prefix nvt=64 | 12.6M | 未測 | −32.93 | — | — | — |
>
> `*` = 生成上限 1024、成對移除截斷樣本後重算。**與 2048 那兩臂不是同一設定，不可直接並列。**
>
> ### ⚠️ 又一次反轉：同設定下 prefix 在安全性上**勝出**
>
> 只有 `lora_qkv` 與 `prefix_nvt8` 是在相同設定（2048）下量的，兩者：
>
> | | 安全 Δ | utility Δ | 參數 |
> |---|---|---|---|
> | LoRA qkv_proj | −2.80 | **+1.22** | 3.15M |
> | prefix nvt=8 | **−5.20** | −4.27 | **1.57M** |
>
> **互不支配**：LoRA 贏 utility、prefix 贏安全性，而且 prefix 只用一半參數。
> 換算成「每百萬參數換到幾個百分點的安全性」，prefix 是 **3.31 vs 0.89——效率高 3.7 倍**。
>
> 這推翻了 P-match 得到的「LoRA 在兩軸全勝」。那個結論成立於
> **nvt=16/64（超出 SVEN 尺度）且安全性用 1024 上限量測**的條件下。
>
> ### 兩臂的退化守門都乾淨通過（首次）
>
> `lora_qkv` 甚至在改善：語法完整率 **+0.40**、stub 率 **−2.80**、無 fence 率 **−10.00**
> （更常使用 code fence）。`prefix_nvt8` 三項也全過（長度保留 97.3%、語法 −0.40、stub −1.60）。
> 這是目前唯二可以「安全性數字單獨回報」的兩臂。
>
> ### 缺口：`lora_qkv` 的 cpp 完全沒有改善
>
> c −5.49 但 **cpp +0.00**。prefix nvt=8 則是 c −5.88 / cpp −4.49，兩種語言都有效。
> `lora_qkv` 的整體 −2.80 幾乎全部來自 c。這在 5 語言的正式評測裡可能被放大或抵銷。
>
> - [x] **P-sven-sec** nvt=8 與 lora_qkv 的安全性
> - [ ] **P-sec-2048** LoRA all-linear 與 prefix nvt=16 的安全性用 2048 重測
>       —— **這是現在的關鍵路徑**。all-linear 的 −17.80 若在 2048 下站得住，
>       它就支配 prefix nvt=8（同樣 −4.27 utility、3.4 倍安全性）；若大幅縮水，
>       整張 Pareto 圖要重畫。它在 1024 下有 14.4% 截斷、ON 程式碼長度 226%，
>       兩者都會被上限影響
> - [ ] **P-sven-attr** nvt=8 無 dropout（歸因）
>
> **四臂完整對照**（2026-09-06，全部在 `--max_new_tokens 2048`、100 題×5、c/cpp、
> base OFF = 49.00 下量測。utility 為 HumanEval 164 題、base OFF = 70.73）：
>
> | | 參數 | 安全 Δ ↓ | utility Δ ↑ | 安全 pt/M參數 | 退化守門 | Pareto |
> |---|---|---|---|---|---|---|
> | **LoRA all-linear** | 12.6M | **−17.60** | −4.27 | 1.40 | ✅ | **✅ 前緣** |
> | **LoRA qkv_proj** | 3.15M | −2.80 | **+1.22** | 0.89 | ✅ | **✅ 前緣** |
> | prefix nvt=8 | **1.57M** | −5.20 | −4.27 | **3.31** | ✅ | ❌ 被 all-linear 支配 |
> | prefix nvt=16 | 3.15M | −6.00 | −10.98 | 1.91 | ❌ 語法 −5.01 | ❌ 被 all-linear 支配 |
> | prefix nvt=64 | 12.6M | 未測 | −32.93 | — | — | — |
>
> ### ✅ LoRA all-linear 的 −17.60 不是截斷假象
>
> 1024 時是 −17.80、截斷 14.4%；2048 時是 **−17.60、截斷 0.00%**。差 0.2 pt，
> 遠小於這個規模的標準誤（~2.2 pt）。**先前對它的疑慮解除。**
>
> ### H1 / H2 確定不成立
>
> **Pareto 前緣上沒有任何 prefix。** 兩個 prefix 尺寸都被 LoRA all-linear 支配：
>
> - prefix nvt=8：utility **−4.27，與 all-linear 完全相同**，但安全性只有 −5.20 vs −17.60（**3.4 倍差距**）
> - prefix nvt=16：兩軸都輸（−10.98 / −6.00 vs −4.27 / −17.60），而且守門沒過
>
> ### prefix 唯一勝出的一項：參數效率
>
> 每百萬參數換到的安全性百分點：**prefix nvt=8 = 3.31**、LoRA all-linear = 1.40、
> LoRA qkv = 0.89。prefix 是 all-linear 的 **2.4 倍**。
> 但這只在「參數預算是硬限制」的情境下才有意義——
> **在 (安全性, utility) 平面上它仍被支配**。
>
> ### prefix 的安全性很快飽和
>
> nvt 8 → 16：參數翻倍、utility 代價從 −4.27 惡化到 −10.98（2.6 倍），
> 安全性卻只從 −5.20 進步到 −6.00（**+0.80**）。
> **多花的容量幾乎全部變成擾動，沒有變成安全性。**
>
> ### 三條線全部隨 prefix 長度單調惡化（機制的三重佐證）
>
> | nvt | 初始擾動 `chosen early`（每 token） | utility Δ | 語法完整率 Δ |
> |---|---|---|---|
> | 8 | −0.4505（98.0%） | −4.27 | −0.40 ✅ |
> | 16 | −0.7700（96.7%） | −10.98 | −5.01 ❌ |
> | 64 | −4.4414（82.3%） | −32.93 | 未測 |
>
> nvt=16 的語法退化在 1024（−4.74）與 2048（−5.01）兩次量測一致，
> 且 2048 下截斷 0.00%，**確認是真實退化不是量測假象**。
>
> ### ⚠️ LoRA all-linear 的數字通過守門，但輸出風格大幅改變
>
> 程式碼長度 **234.8%**、行數 +16.95、無 fence 率 80.8% → **98.6%**，
> 加上先前量到的註解率 18.9% → **48.6%**。
> 守門的三條件擋的是「寫太少 / 寫壞 / 寫 stub」，**擋不住「寫太多」**。
> 通過守門只代表沒有用那三種方式作弊，不代表 −17.60 全部來自「學會安全寫法」。
> 抽樣可見它會**在註解裡寫安全建議、然後照樣寫出漏洞碼**（見 D-screen2 的樣本）。
> 這一點應在論文中單獨討論，不要當成純粹的勝利。
>
> - [x] **P-sec-2048** 四臂安全性統一在 2048 下量測完成
> - [ ] **正式評測** 693 題 × 10 samples × 5 語言（決定要報哪幾臂之後）
> - [ ] **P-sven-attr** nvt=8 無 dropout（歸因：長度 vs dropout）
> - [ ] **MultiPL-E** 各臂的多語言 utility
>
> **全量評測結果**（2026-09-07，693 題 × 10 樣本 × 5 語言、`--max_new_tokens 2048`、
> 四臂共用一份 OFF。n=6,930/臂，標準誤 **0.60 pt**，所有效果都顯著 4.3~23.9 SE）：
>
> 依論文的算法（逐語言漏洞率再對 5 語言取平均）：
>
> | 臂 | c | cpp | java | js | py | **平均** | **Δ** | 相對 | 守門 |
> |---|---|---|---|---|---|---|---|---|---|
> | OFF (base) | 69.41 | 26.62 | 61.50 | 51.53 | 30.64 | **47.94** | — | — | — |
> | **LoRA all-linear** | 50.59 | **8.38** | 50.59 | 36.35 | 22.57 | **33.70** | **−14.24** | −29.7% | ⚠️ py 未過 |
> | LoRA qkv_proj | 66.08 | 22.97 | 60.21 | 47.53 | 30.03 | 45.36 | −2.58 | −5.4% | ✅ 全過 |
> | prefix nvt=16 | 59.22 | 22.70 | 54.81 | 43.76 | 28.99 | 41.90 | −6.04 | −12.6% | ❌ 未過 |
> | prefix nvt=8 | 66.67 | 24.19 | 55.56 | 48.47 | 29.83 | 44.94 | −3.00 | −6.2% | ✅ 全過 |
>
> ### 🎯 論文的安全性數字復現成功
>
> | | base | after | Δ | 相對 |
> |---|---|---|---|---|
> | **論文 ProSec（SimPO）** | 50.57 | **33.47** | −17.10 | −33.8% |
> | **我們 LoRA all-linear（DPO）** | 47.94 | **33.70** | −14.24 | −29.7% |
>
> **ON 的絕對值只差 0.22 pt**，而且我們用的是 DPO、論文用 SimPO。
> base 低 2.63 pt 來自評測子集重建的差異（36 組 vs 38 組）。
>
> **但逐語言差異很大，平均吻合有一部分是巧合**：
> C 我們較差（50.59 vs 44.27）、JS 較差（36.35 vs 28.21）、
> **C++ 我們好很多（8.38 vs 20.74）**。論文裡要把逐語言的散布一起報，不能只報平均。
>
> ### 便宜篩選的數字全部要以這一輪為準
>
> | | c/cpp 篩選（100題×5） | 全量（5 語言平均） |
> |---|---|---|
> | LoRA all-linear | −17.60 | **−14.24** |
> | LoRA qkv_proj | −2.80 | **−2.58** |
> | prefix nvt=16 | −6.00 | **−6.04** |
> | prefix nvt=8 | −5.20 | **−3.00** |
>
> c/cpp 高估了 all-linear 與 prefix nvt=8（c 的 base 69% 最有改善空間）。
> qkv 與 nvt=16 則相當穩定。**篩選只能看方向，不能當結論**——這一輪證實了這條規則。
>
> ### Pareto 前緣仍然全是 LoRA
>
> | 臂 | 安全 Δ | utility Δ | |
> |---|---|---|---|
> | LoRA all-linear | −14.24 | −4.27 | ✅ 前緣 |
> | LoRA qkv_proj | −2.58 | +1.22 | ✅ 前緣 |
> | prefix nvt=8 | −3.00 | −4.27 | ❌ 被 all-linear 支配 |
> | prefix nvt=16 | −6.04 | −10.98 | ❌ 被 all-linear 支配 |
>
> prefix nvt=8 付出**與 all-linear 完全相同的 utility 代價（−4.27）**，
> 只買到五分之一的安全性。H1/H2 在全量評測下仍然不成立。
>
> prefix 唯一勝出的仍是**參數效率**：nvt=8 每百萬參數 1.91 pt，all-linear 1.13 pt。
>
> ### 退化守門：只有兩臂全過
>
> | 臂 | 整體 | 各語言 |
> |---|---|---|
> | LoRA qkv_proj | ✅ | 五種語言全過，且語法完整率**上升**、無 fence 率降 11.31 |
> | prefix nvt=8 | ✅ | 五種語言全過（程式碼長度保留 100.1%） |
> | LoRA all-linear | ✅ 整體 −2.61 | **python 未過（−5.07）** |
> | prefix nvt=16 | ❌ −3.76 | c / cpp / java / python 皆未過，只有 js 過 |
>
> **LoRA all-linear 的 python 安全性數字（22.57）不得單獨回報**，必須並列退化指標。
> 它整體的程式碼長度是 base 的 **226.4%**、無 fence 率 82.77% → **97.22%**——
> 守門三條件擋不住「寫太多」，這一點必須在論文中討論。
>
> - [x] **全量安全性評測** 四臂完成
> - [ ] **MultiPL-E** 四臂的 js/cpp/java（補齊論文 HumanEval-Multi 的另外三欄）
> - [ ] **P-sven-attr** nvt=8 無 dropout（歸因）
>
...
> **⚠️ 2026-09-08 再修訂：量了 SVEN 的資料集，H3 的前提明顯減弱。**
> （原寫「基本崩了」，2026-09-09 同尺重量後修正為「明顯減弱」）
>
...
> 遠超過單位誤差）：
>
> **⚠️ 2026-09-09 更正：上表的單位混用，把差距誇大了。**
> SVEN 的 0.8% 是它自己標註的**最小字元編輯**（`char_changes`，只算 ` %`→`,`
> 那兩個字元），ProSec 的 35.8% 是 `difflib` 在 **token** 上算的粗顆粒 diff。
> 兩把尺精細度不同，不能相除。用**同一套**（Phi-3 tokenizer + `difflib`，
> 即 `edit_locality.py` 的 ρ 定義）重量 SVEN 720 筆：
>
> | | SVEN | ProSec | 差距 |
> |---|---|---|---|
> | token 數（p50） | 409 | — | — |
> | **ρ 改動占比（p50）** | **0.058** | **0.358** | **6.2×** |
> | **hunk 平均** | **4.1** | **17.0** | **4.1×** |
> | **單一連續 diff** | **21.0%** | **0.1%** | — |
>
> **正確的量級是約 6 倍，不是「一到兩個數量級」。**
> 且 SVEN 的函式偏長（p50 409 token、mean 1152，真實 CVE commit 的 C/Python
> 函式），同樣改一行在長函式裡 ρ 自然較小——6 倍裡有一部分是長度效應。
> **hunk 那兩列比較站得住**（4.1 vs 17、21% vs 0.1%），它們量的是「散不散」，
> 受函式長度影響較小，且與 ρ 同向。
>
> 結論方向不變（SVEN 像修補、ProSec 像重寫），**強度降一級**。
...
> **⚠️ 2026-09-09 決定性結果：ProSec 裡沒有 patch-like 子集，S5 masking 放棄。**
>
> 假設是「ProSec 是混合體，CWE-338（弱亂數，修補通常是一行替換）之類的子集
> 應該接近 SVEN 的型態」。`data/rho_by_cwe.py` 依 CWE 拆開量（同 ρ 定義）：
>
> | CWE | 筆數 | ρ p50 | hunk | 單一 hunk |
> |---|---|---|---|---|
> | CWE-119 | 260 | 0.175 | 16.5 | 0.0% |
> | CWE-89 | 636 | 0.193 | 20.9 | 0.2% |
> | **CWE-338** | **21,254** | **0.292** | **18.6** | **0.1%** |
> | CWE-78 | 7,339 | 0.312 | 19.9 | 0.4% |
> | CWE-502 | 8,655 | 0.411 | 24.0 | 0.0% |
> | CWE-22 | 5,367 | 0.549 | 24.5 | 0.0% |
> | CWE-352 | 420 | 0.715 | 25.6 | 0.0% |
> | *SVEN* | *720* | *0.058* | *4.1* | *21.0%* |
>
> **ρ 跨 4 倍（0.175~0.715），但 hunk 數幾乎不變（16.5~29.0），單一連續 diff
> 在每個 CWE 都趨近 0。** CWE-338 是 0.292，不是預期的 patch-like。
>
> 解釋：散落是**結構性**的，來自「LLM 重新生成整個函式」這個資料產生方式，
> 而非漏洞類型。語意上的修補再小，重新生成都會把 token 洗遍全文。
>
> **S5（masked DPO）正式放棄**，理由比先前更硬：不是 ρ 太高，而是**任何 CWE 的
> diff 都碎成約 20 片**，從中建出的 mask 是紙屑不是區域。PTC 的 Table III 顯示
> masking 在極小編輯資料上值 +31.7 CSR，但那個前提在 ProSec 裡任何一個子集都
> 不成立。`S5-bucket` 一併取消（低 ρ 分桶同樣散落）。
>
> **意外收穫**：查評測集 CWE 分佈時發現**訓練 12 個 CWE、評測 50 個**，
> 前 15 名（76.4% 題數）裡只有 5 個在訓練集內。也就是說 ProSec 測到的安全性
> 改善**大部分是泛化到沒見過的 CWE**，而非學會訓練標的——ProSec 論文沒拆開報。
> 而 prefix 與 LoRA 的泛化能力是否相同**沒有人量過**，且拆帳**不需要新訓練或
> 新生成**（見下方 L-gen）。

> **🔴 2026-09-13 重大發現：nvt=16 / 64 兩臂的有效 prefix 長度是 1。**
>
> 零初始化把 K 和 V 全設 0，於是 nvt 個位置在 step 0 完全相同。由對稱性它們收到
> 相同的梯度，Adam 的 m/v 初始也是 0，更新量也相同——**沒有任何隨機來源的話，
> 它們永遠保持相同**。`eval/check_prefix_symmetry.py` 直接讀存下來的
> `prompt_embeddings` 張量驗證：
>
> | 臂 | dropout | 列間最大相對差 | **有效列數** |
> |---|---|---|---|
> | `prefix`（nvt=16） | **無** | 8.4e-04 | **1.01 / 16** |
> | `prefix_nvt8` | **0.1** | 4.6e-01 | **4.84 / 8** |
> | `prefix_nvt64` | **無** | 1.8e-03 | **1.03 / 64** |
>
> SVEN 預設就有 dropout（`sven/model.py:20`），所以論文本身沒踩到；我們的
> nvt=16 / 64 沒開，踩了。
>
> **作廢的結論**：
> - ❌ **「prefix 越長：安全性越好、功能性掉越多」不能再解釋成長度效應。**
>   nvt=16 / 64 學到的內容與 nvt=1 相同，但 attention 吸收隨 nvt 上升
>   （`Z/(Z+nvt)`）。所以那條單調趨勢量到的是「**零初始化位置越多、擾動越大、
>   學到的東西沒變多**」。簡報上第二點要改寫。
> - ❌ nvt=16 vs nvt=64 的「容量對照」不成立（兩者容量都是 1）。
> - ❌ `prefix`(nvt=16) 的 seen/unseen 比值 0.46 意義有限（那是被廢掉的臂）。
>
> **仍然成立的結論**：
> - ✅ **主結果不受影響**：headline 用的是 `prefix_nvt8`，它有 dropout、有效列數
>   4.84，是正常訓練出來的。prefix nvt=8 vs LoRA 的比較、以及它的 seen/unseen
>   比值 1.12（泛化）都站得住。
> - ✅ 零初始化不是 no-op 的機制（attention 吸收）不受影響，反而被強化：
>   **傷害隨 nvt 上升、能力卻停在 1**，兩者完全分離。
>
> **一個耐人尋味的觀察**：`prefix`(nvt=16) 有效容量只有 1，卻拿到比 nvt=8 更多的
> 安全性（−6.04 vs −3.00）。這暗示那些臂的「安全性改善」有相當部分來自
> **全域壓低 attention**，而不是學到的內容——而同一個機制也正是 utility 的代價。
>
> **補救（最高優先，一輪 3.5 小時）**：`ARMS=prefix16drop bash scripts/run_sven_scale.sh`
> 一次補上兩個乾淨對照——對 `prefix_nvt8` 只差長度、對 `prefix`(nvt=16) 只差 dropout。
> `train_prefix.py` 已加守門，之後這個設定會直接拒絕執行。

#### 前置分析（必須先做，幾小時，可省下一週實作）
- [x] **S5-loc** edit-locality 統計完成（見 S1-loc 結果）
- [ ] **S5-overlap** **diff 與 analyzer finding 行號的重疊率** —— **現在是 S5 的成敗關鍵**
- [x] ~~**S5-bucket** 依 ρ 分桶~~ —— 2026-09-09 **取消**：逐 CWE 拆開後
      發現低 ρ 的 CWE 同樣散成 16~20 個 hunk，沒有真正局部的子集
- [x] **S5-cwe** 依 CWE 量 ρ（見上方 2026-09-09 結果）
- [ ] **L-gen** seen vs unseen CWE 拆帳（`eval/score_generalization.py`）
      —— **零成本、最高價值**：資料已算好，只需重新拆帳。
      量 prefix 與 LoRA 的泛化差異，這是兩篇 prefix 論文都沒碰的問題
...
**checkpoint 快篩（HumanEval，base 70.73%）：早停救不回來。**

| checkpoint | pass@1 | Δ | 撞 2048 上限 |
|---|---|---|---|
| 100 | 50.0% | −20.73 | 1/164（可信） |
| 200 | 6.71% | −64.02 | 40/164（已崩） |
| 800 | 3.66% | −67.07 | 8/164（已崩） |

崩潰發生在 step 100~200 之間。step 100 的 −20.73 不是截斷造成的，已經比
`prefix_nvt8` 差得多。
...
### MLP prefix 的最終結論（2026-09-19）

`prefix_mlp_ckpt100`（lr 1e-5 那次的 checkpoint-100，step 800 已崩）全量評測：

| 臂 | 安全性 Δ | 功能性 Δ | 安全性 / 功能性代價 |
|---|---|---|---|
| `lora_mlp` | −5.86 | −1.64 | 3.57 |
| `lora_all` | −14.24 | −4.12 | 3.46 |
| **`prefix_mlp_ckpt100`** | **−9.14** | **−18.37** | **0.50** |
| `prefix_nvt8` | −3.00 | −7.22 | 0.42 |

（功能性：HumanEval −9.76、js −25.47、cpp −19.88。退化守門整體通過，java 因程式碼
長度保留 86.9% 未過。）

**結論：MLP 重參數化沒有改善取捨，只是把 prefix 推到曲線更遠處。** 0.50 vs 0.42 在
雜訊內（功能性每語言誤差 ±2~3 pt），但操作點差很多。與 lr 那次同一個形狀——
改變的是走多遠，不是走在哪條線上。prefix 的結果仍以 `prefix_nvt8` 為準。
...
### prefix 家族的最終結論（2026-09-20）

`phi3-prefix-simpo-full` 全量評測：安全性 **−3.41**（5 語言平均）、功能性 **−6.20**
（HumanEval −1.83、js −9.32、cpp −7.45）。⚠️ HumanEval 再次低估（−1.83 vs −6.20）。

**四個完整評測的 prefix 臂，全部落在同一條取捨線上：**

| 臂 | 目標函數 | nvt | 其他 | 安全性 | 功能性 | 效率 |
|---|---|---|---|---|---|---|
| `prefix` | DPO | 16 | 無 dropout | −6.04 | −16.91 | 0.36 |
| `prefix_nvt8` | DPO | 8 | dropout 0.1 | −3.00 | −7.22 | 0.42 |
| `prefix_mlp_ckpt100` | DPO | 8 | dropout + MLP | −9.14 | −18.37 | 0.50 |
| `prefix_simpo` | **SimPO** | 16 | 無 dropout、1024/512 | −3.41 | −6.20 | **0.55** |
| *LoRA 各臂* | DPO | — | — | — | — | *3.46~3.57* |

跨越兩種目標函數、三種長度、有無 dropout、有無 MLP 重參數化——**效率全部在
0.36~0.55，LoRA 是 3.5，差約 7 倍**。每次改旋鈕只是在同一條線上移動位置，
沒有任何一次把線本身推出去。

**結論：停止調 prefix。** 再訓「設定正確」的 SimPO prefix（nvt=8 + dropout +
2048/1024）預期落在 0.5~0.6，仍是 LoRA 的六分之一，不值得 3.5h 訓練 + 6h 評測。
下一步是 PT-PEFT 的組合實驗（`scripts/run_pt_peft.sh`，程式已完成）。

### LoRA + SimPO checkpoint 掃描（2026-09-21）

照論文 Table 6 的 SimPO 設定（all-linear、β=1.5、γ=0.5、lr 5e-6、1500 步、clip 1.0）。

| checkpoint | 安全性 Δ | 功能性 | 判定 |
|---|---|---|---|
| 300 | **−2.39** | **−0.18**（HumanEval −3.66 / js +3.73 / cpp −0.62） | 可用但安全性太弱 |
| 600 | 未測 | HumanEval **35.37%（−35.37）** | 已退化 |
| 1500 | 未測 | HumanEval 3.66%（−67.07） | 崩潰 |

對照（DPO、800 步、clip 0.3）：`lora`(all-linear) −14.24 / −4.12、`lora_qkv` −2.58 / +1.03。

**判讀**：step 300 的 SimPO 功能性幾乎零成本，但安全性只有 −2.39——同樣是 all-linear，
DPO 走 800 步拿到 −14.24。所以這不是「SimPO 不如 DPO」，是**訓練量不夠**。
軌跡兩端已知（300 太弱、1500 崩），中間還有 600/900/1200。

**結論：SimPO 在我們的設定上沒有可用的操作點能勝過 DPO。** 可用窗口只在 300 附近，
那裡安全性僅 −2.39；600 步功能性已掉 35.4 pt，崩潰來得很快。同一 LoRA 配置下 DPO
在 800 步達到 −14.24 / −4.12。

300~600 之間（間隔 300）未探，要重訓才拿得到 400/500 的 checkpoint，合計約 9 小時，
